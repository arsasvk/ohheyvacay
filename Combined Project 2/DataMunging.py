...
DB1B_dataDF = DB1B_dataDF_new.drop(["ORIGIN_COUNTRY", "ORIGIN_STATE_ABR", "DEST_STATE_ABR", "Code"], axis=1)
...
#### removing the rows where the MARKET_FARE has < 50 value. 

cleanDB1B_dataDF  = DB1B_dataDF_origin_dest_clean_new[DB1B_dataDF_origin_dest_clean_new.MARKET_FARE > 50]  ## reduced to 928750 rows.

##### Drop more columns that are not needed to reduce DB space.
cleanDB1B_dataDF_new =  cleanDB1B_dataDF.drop(["ORIGIN_AIRPORT_ID", "DEST_AIRPORT_ID", "DEST_STATE_NM", "ORIGIN_STATE_NM"], axis=1) 
